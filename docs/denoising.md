...
The denoising module reduces noise in tomograms while preserving structural detail. It’s typically the **first step** before semantic/instance segmentation or particle identification. For best performance, it's recommended to have a WBP reconstructed tomogram, since the denoising model was trained on WBP simulated tomograms. 
...
You can download the trained model from here: [CryoSiam denoising model (v1.0)](https://huggingface.co/frosinastojanovska/cryosiam_v1.0/blob/main/cryosiam_denoising.ckpt)
...
**What it does**  
- Loads the trained denoising model and your tomograms  
...
```yaml
data_folder: '/scratch/stojanov/empiar_datasets/EMPIAR-11756/tomograms'
...
| `trained_model` | `str` |                             ✅ | Path to the denoising checkpoint file (e.g., `.ckpt`).                                          |
| `file_extension` | `str` |                             ❌ | Extension of input tomograms (`.mrc` or `.rec`).                                                |
| `test_files` | `list[str]` or `null` |                             ❌ | List of specific filenames to process. Use `null` to process **all** files in `data_folder`.    |
| `save_raw_predictions` | `bool` |                             ❌ | If `true`, also save the raw network output before any scaling/post‑processing.                 |
| `scale_prediction` | `bool` |                             ❌ | If `true`, rescale/restore prediction to the target intensity range (recommended for viewing/export). |
...
> **Tips**  
> • Keep `patch_size` divisible by the network downsampling factor (e.g., 128/64/32).
...
- Continue with [Semantic segmentation](semantic.md), [Instance segmentation](instance.md) or [Particle identification](particle_identification.md)
